...
// incluso al recargar la página directamente en /estadisticas.
//
// También observa cambios en data-theme para actualizar colores en vivo.

import { useEffect, useState, useCallback } from "react";

// Mapa de respaldo directo: tema → colores para gráficas.
// Debe mantenerse sincronizado con los --chart-color-N de globals.css.
...
function coloresIniciales(count: number): string[] {
    ...
    // Intenta leer de CSS por si es un tema nuevo no registrado en el mapa
    const delCSS = leerColoresCSS(count);
    if (delCSS.length > 0) return delCSS;

    // Fallback absoluto — strawberry (el tema original de la app)
...
export function useChartColors(count: number = 6): string[] {
    ...
    const sincronizar = useCallback(() => {
        // Prioridad 1: leer de CSS (futuro-proof para temas nuevos)
        const delCSS = leerColoresCSS(count);
        if (delCSS.length > 0) {
            setColors(delCSS);
            return;
        }
        // Prioridad 2: mapa de respaldo
        const tema = obtenerTema();
        const delMapa = COLORES_POR_TEMA[tema]?.slice(0, count);
