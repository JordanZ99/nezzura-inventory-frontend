...
export default function ReporteHeader({ logoSrc, empresa, dates, periodoLabel }: Props) {
    return (
        <div className="flex md:hidden" style={{ alignItems: "center", gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: "2px solid var(--border-primary)" }}>
            ...
            <div>
                ...
                <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--text-muted)" }}>
                    Período: {periodoLabel ?? `${dates.from ? dates.from.toLocaleDateString() : "Inicio"} — ${dates.to ? dates.to.toLocaleDateString() : new Date().toLocaleDateString()}`}
                </p>
            </div>
        </div>
    )
